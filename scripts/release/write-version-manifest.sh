...
jq -n \
  ...
  '{application:$application,license:"AGPL-3.0-only",release_sha:$release_sha,source_offer:$source_offer,images:{backend:$backend,frontend:$frontend,caddy:$caddy,postgres:$db,auth:$auth,rest:$rest},migration_version:$migration,opentofu_version:$opentofu,generated_at:$generated_at}' \

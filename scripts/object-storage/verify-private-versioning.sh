...
[[ "$probe_host" == "$SSE_C_PROBE_ALLOWED_HOST" && "$SSE_C_PROBE_ACCOUNT_ID" != "$AWS_ACCOUNT_ID" ]] || { printf 'Scratch object target is not isolated.\n' >&2; exit 1; }
...
cleanup() {
  if [[ -n "$version_id" ]]; then
    aws s3api delete-object --bucket "$SSE_C_PROBE_BUCKET_NAME" --key "$key" --version-id "$version_id" --endpoint-url "$SSE_C_PROBE_ENDPOINT_URL" >/dev/null 2>&1 || true
  fi
  residue=$(aws s3api list-object-versions --bucket "$SSE_C_PROBE_BUCKET_NAME" --prefix "$key" --endpoint-url "$SSE_C_PROBE_ENDPOINT_URL" --output json 2>/dev/null || printf '{}')
...
preexisting=$(aws s3api list-object-versions --bucket "$SSE_C_PROBE_BUCKET_NAME" --prefix "$key" --endpoint-url "$SSE_C_PROBE_ENDPOINT_URL" --output json)
[[ "$(jq '[.Versions[]?, .DeleteMarkers[]?] | length' <<<"$preexisting")" == "0" ]] || { printf 'Probe key already has residue.\n' >&2; exit 1; }
put_result=$(aws s3api put-object --bucket "$SSE_C_PROBE_BUCKET_NAME" --key "$key" --body "$body" --endpoint-url "$SSE_C_PROBE_ENDPOINT_URL" --sse-customer-algorithm AES256 --sse-customer-key "fileb://$SSE_C_CUSTOMER_KEY_FILE")
version_id=$(jq -er '.VersionId | select(type == "string" and length > 0)' <<<"$put_result")
if aws s3api get-object --bucket "$SSE_C_PROBE_BUCKET_NAME" --key "$key" --endpoint-url "$SSE_C_PROBE_ENDPOINT_URL" "$without_key" >/dev/null 2>&1; then
  printf 'A keyless read unexpectedly succeeded; SSE-C is not enforced.\n' >&2
  exit 1
fi
aws s3api get-object --bucket "$SSE_C_PROBE_BUCKET_NAME" --key "$key" --endpoint-url "$SSE_C_PROBE_ENDPOINT_URL" --version-id "$version_id" --sse-customer-algorithm AES256 --sse-customer-key "fileb://$SSE_C_CUSTOMER_KEY_FILE" "$with_key" >/dev/null
cmp -s "$body" "$with_key"

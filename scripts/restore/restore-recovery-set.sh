...
: "${RESTORE_COMPOSE_FILE:?set RESTORE_COMPOSE_FILE to the production Compose topology}"
: "${RESTORE_ENV_FILE:?set RESTORE_ENV_FILE to the disposable mode-600 env file}"
: "${RESTORE_PROJECT_NAME:?set RESTORE_PROJECT_NAME to a unique disposable project name}"
...
: "${RESTORE_OBJECT_BUCKET:?set RESTORE_OBJECT_BUCKET to the disposable bucket}"
...
for secret_file in "$AGE_IDENTITY_FILE" "$RESTORE_ENV_FILE" "$RESTORE_OBJECT_SSE_CUSTOMER_KEY_FILE"; do
  [[ -f "$secret_file" ]] || { printf 'Missing restore secret file: %s\n' "$secret_file" >&2; exit 1; }
  [[ "$(stat -c '%a' "$secret_file")" == "600" ]] || { printf 'Restore secret file must be mode 600: %s\n' "$secret_file" >&2; exit 1; }
done
[[ -f "$RECOVERY_SET_PATH" && -f "$RECOVERY_SUCCESS_PATH" ]] || { printf 'Recovery archive or success marker is missing.\n' >&2; exit 1; }
case "$(basename "$RESTORE_COMPOSE_FILE")" in
  docker-compose.yml|docker-compose.yaml)
    printf 'Refusing to restore with the local demo Compose file.\n' >&2
    exit 1
    ;;
esac

mkdir -p "$RESTORE_ROOT"
runtime_dir=$(mktemp -d "$RESTORE_ROOT/restore.XXXXXX")
archive="$runtime_dir/recovery.tar.gz"
set_dir="$runtime_dir/set"
mkdir -p "$set_dir" "$RESTORE_ROOT/restore-receipts"

age --decrypt --identity "$AGE_IDENTITY_FILE" --output "$archive" "$RECOVERY_SET_PATH"
tar -xzf "$archive" --no-same-owner -C "$set_dir"
...
"${compose[@]}" up -d auth rest backend frontend
...
if [[ -n "${RESTORE_EXPECTED_COUNTS_FILE:-}" ]]; then

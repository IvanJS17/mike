...
: "${RELEASE_MANIFEST_PATH:?set RELEASE_MANIFEST_PATH=/srv/litt-data/state/release-manifest.json}"
...
[[ "$(realpath -m "$RELEASE_MANIFEST_PATH")" == "/srv/litt-data/state/release-manifest.json" && -f "$RELEASE_MANIFEST_PATH" && ! -L "$RELEASE_MANIFEST_PATH" && "$(stat -c '%a' "$RELEASE_MANIFEST_PATH")" == "600" ]] || { printf 'Freshness release manifest is not canonical mode 600.\n' >&2; exit 1; }
release_sha=$(jq -er '.release_sha | select(type == "string" and test("^[0-9a-f]{40}$"))' "$RELEASE_MANIFEST_PATH")
[[ "$(git -C "$root" rev-parse HEAD)" == "$release_sha" && -z "$(git -C "$root" status --porcelain --untracked-files=all)" ]] || { printf 'Freshness checkout does not match clean release manifest.\n' >&2; exit 1; }
...
' "$tmp" >/dev/null 2>&1 || fail_closed "latest-success marker is incomplete or from another release"

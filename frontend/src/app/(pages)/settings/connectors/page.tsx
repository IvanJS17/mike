    ...
    const connectConnectorOAuth = async (
        ...
        try {
            await new Promise<void>((resolve, reject) => {
                ...
                // Self-rescheduling poll rather than a fixed setInterval. Two
                // reasons: (1) we back the cadence off from 1.5s to 5s after the
                // first minute — the happy path resolves in seconds, so a user
                // slowly reading a consent screen shouldn't generate ~200
                // authenticated GETs over the five-minute window; (2) chaining
                // the next poll only after the previous read settles guarantees
                // we never stack requests on a slow connection.

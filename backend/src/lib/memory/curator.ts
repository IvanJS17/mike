...
const defaultCuratorScopeServicesNote =
  "LITT (S5b): the curator's provider egress is governed — the default stream is" +
  " built per run from the actor context (curatorEgress.ts) and fails closed" +
  " while EGRESS_POLICY allowlists only the fake host.";
...
export async function runMemoryCuratorScope(
  ...
  services?: CuratorScopeServices,
): Promise<CuratorScopeOutcome> {
  void defaultCuratorScopeServicesNote;
  services ??= {
    stream: createGovernedCuratorStream({
      db: args.db,
  ...
  try {
    ...
    if (error instanceof MemoryCuratorEgressBlockedError) {
      // Governance blocks are terminal and carry only safe, typed detail
      // (kind + governed message — never prompt, key or transcript material).
      throw error;
    }
    void error;
...
export async function handleMemoryConsolidation(
  ...
  for (const candidate of files) {
    ...
    try {
      ...
      if (error instanceof MemoryCuratorEgressBlockedError) {
        // LITT (S5b): governed-egress block is terminal for this run — zero
        // provider calls, zero retries. Record it as a failed consolidation
        // instead of a transient scope failure.
        outcomes[candidate.file.scope] = "egress_blocked";
        await setStatus({
          db,
          stateId,
          generation: requestedGeneration,
          status: "failed",
          errorCode: "curation_egress_blocked",
        });
        await refreshJobFileStatuses({
          db,
          job,
        ...
        return { skipped: "egress_blocked", outcomes };
      }

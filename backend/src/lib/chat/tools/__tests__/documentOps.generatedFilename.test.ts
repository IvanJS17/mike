...
describe("safeGeneratedFilename", () => {
  ...
  // es-MX legal titles: accents, ordinal abbreviations and multi-word names
  // survive NFC naming; punctuation such as the period in "núm." is dropped.
  it.each([
    ["Resolución", "Resolución.docx"],
    ["Demanda núm. 3", "Demanda núm 3.docx"],
    ["Contrato de prestación", "Contrato de prestación.docx"],
  ])("keeps es-MX legal titles readable: %s", (title, expected) => {
    expect(safeGeneratedFilename(title, "docx")).toBe(expected);
  });

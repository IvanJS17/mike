...
describe("project access grants", () => {
    ...
    it("rejects a grant when the email does not belong to an existing user", async () => {
        ...
        await expect(
            ...
            ok: false,
            kind: "validation",
            detail: "future@firm.example does not belong to a LiTT user.",
        });
...
describe("project Owner contacts", () => {
    it("uses only effective organization Owners for an organization project", async () => {
        const db = makeDb({
            ...
            organization_memberships: [
                { organization_id: "o1", user_id: "creator", role: "org_owner", status: "active" },
                { organization_id: "o1", user_id: "boss", role: "workspace_admin", status: "active" },
                { organization_id: "o1", user_id: "staffer", role: "editor", status: "active" },
            ],
            project_org_access_overrides: [
                { project_id: "p1", org_id: "o1", user_id: "boss", role: "owner" },
            ],
            user_profiles: [
                {
                    user_id: "creator",
        ...
        // members without an explicit Owner override are not someone to ask
        // for access.
    ...
    it("still names an explicit Owner override when the creator's account is gone", async () => {
        // An org project survives its creator: with the creator absent, the
        // firm's explicit Owner-override holder is who you ask.
        const db = makeDb({

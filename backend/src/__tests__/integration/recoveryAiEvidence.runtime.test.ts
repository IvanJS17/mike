...
import { IDS, LEGACY_IDS, SEED, LEGACY_AI_SEED } from "./fixtures/recoveryLegacyEvidence";
...
function expectTableFingerprintsEqual(upgradedDatabase: string): void {
  const incremental = tableFingerprint(upgradedDatabase);
...
maybe("Slice E2a disposable PostgreSQL runtime", () => {
  ...
  it("converges the exact AI table fingerprint and has no legacy Drive objects", () => {
    // Retain the E2a fixture for its historical RPC regressions below, but
    // compare current fresh tables against the explicit current upgrade.
    const current = "e2a_current_artifact";
    docker(["createdb", "-U", "postgres", "-T", INCREMENTAL, current]);
    apply(current, file(path.join(MIGRATIONS_DIR, "20260905_02_recovery_approved_artifact_storage.sql")));
    expectTableFingerprintsEqual(current);
    expect(
      psql(
        INCREMENTAL,

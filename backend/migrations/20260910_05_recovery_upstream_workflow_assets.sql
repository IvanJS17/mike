...
do $$
begin
  if to_regclass('public.mike_workflow_reference_files') is not null then
    if to_regclass('public.mike_workflow_assets') is null then
      alter table public.mike_workflow_reference_files
        rename to mike_workflow_assets;
    elsif not exists (
      select 1 from public.mike_workflow_reference_files
    ) then
      -- A full-manifest replay re-creates this relation empty after this
      -- migration already renamed it (earlier migrations guard their DDL
      -- with `if not exists`); the canonical post-migration state has no
      -- such table. Never drop a populated legacy relation.
      drop table public.mike_workflow_reference_files;
    end if;
  end if;
end
$$;
...
begin
  ...
  if exists (
    select 1 from pg_constraint
    where conname = 'mike_workflow_reference_files_name_unique'
      and conrelid = 'public.mike_workflow_assets'::regclass
  ) then
    alter table public.mike_workflow_assets
      rename constraint mike_workflow_reference_files_name_unique
      to mike_workflow_assets_name_unique;
  end if;
end
$$;

...
const execution = {
  ...
  document_content_sha256: sha("synthetic-docx"),
  status: "succeeded",
  error_class: null,
...
const review = {
  ...
  evidence_receipt_sha256: sha("receipt"),
  items: [],
};
function bundle(
  ...
  const actions = spans.map(([start, end], index) => ({
    ...
    review_item_id: `item-${index}`,
    citation_id: `citation-${index}`,
    document_id: execution.document_id,
    document_version_id: execution.document_version_id,
    page: 1,
    start,
    end,
    page_content_sha256: sha(text),
    before_text_sha256: sha(text.slice(start, end)),
  ...
  const body = {
    ...
    ...patch,
    actions: actions.map(
      ({ replacement_text: _text, ...identity }) => identity,
    ),
  };
  ...
  return {
    ...
    actions,
    canonical_json,
    bundle_sha256: sha(canonical_json),
  };
}
...
async function setup(addin: Addin, page: Page, documentText = text) {
  ...
  await page.addInitScript(() => {
    document.addEventListener("DOMContentLoaded", () => {
      ...
      host.Office.context.document.getFileAsync = (
        _type: unknown,
        _options: unknown,
        callback: any,
      ) => {
        ...
        const bytes = Array.from(
          new TextEncoder().encode(host.__SNAPSHOT_BYTES__),
        );
        callback({
          status: host.Office.AsyncResultStatus.Succeeded,
          value: {
            sliceCount: 1,
            getSliceAsync: (index: number, done: any) =>
              done({
                status: host.Office.AsyncResultStatus.Succeeded,
                value: { index, data: bytes },
              }),
            closeAsync: (done?: any) =>
              done?.({ status: host.Office.AsyncResultStatus.Succeeded }),
          },
        });
      };
    });
  ...
  await addin.mockApiJson("GET", "**/projects?*", [
    { id: "project-1", name: "Synthetic matter" },
    { id: "project-2", name: "Other matter" },
  ]);
  await addin.mockApiJson("GET", "**/projects/project-1/ai-executions", [
    execution,
    { ...execution, id: "failed-1", status: "failed" },
  ]);
...
async function load(page: Page) {
  await page
    .getByLabel("Execution", { exact: true })
    .selectOption("execution-1");
}
...
async function saveAs(page: Page) {
  await page.evaluate(() => {
    (window as any).Office.context.document.url = "C:/copy.docx";
  });
}

test("happy load and descending single-action gating", async ({
  addin,
  page,
}) => {
  ...
  await expect(
    page.getByRole("button", { name: "Apply action-2", exact: true }),
  ).toBeEnabled();
  await expect(
    page.getByRole("button", { name: "Apply action-1", exact: true }),
  ).toBeDisabled();
  await expect(
  ...
  await zeroWrites(addin);
  await page
    .getByRole("button", { name: "Apply action-2", exact: true })
    .click();
  ...
  await page
    .getByRole("button", { name: "Apply action-1", exact: true })
    .click();
  await expect(cards.nth(1)).toContainText("Applied");
  const calls = await addin.wordCalls();
  expect(calls.trackedChanges.map((call) => call.original)).toEqual([
    "Beta clause.",
    "Alpha clause.",
  ]);
});
...
for (const field of [
  ...
  test(`rejects cross-scope review ${field}`, async ({ addin, page }) => {
    await setup(addin, page);
    await addin.mockApiJson("GET", scopeUrl, {
      ...review,
      [field]: field.endsWith("sha256") ? sha("other") : "other",
    });
    await load(page);
...
for (const failure of [
  ...
  test(`load fails closed: ${failure}`, async ({ addin, page }) => {
    await setup(
      addin,
      page,
      failure === "ambiguity" ? `${text} Alpha clause.` : text,
    );
    if (failure === "tamper") {
      const value = bundle();
      value.actions[0]!.replacement_text = "tampered";
      await addin.mockApiJson("GET", bundleUrl, value);
    }
    if (failure === "scope")
      await addin.mockApiJson("GET", bundleUrl, bundle({ matter_id: "other" }));
    if (failure === "source")
      await addin.mockApiJson(
        "GET",
        bundleUrl,
        bundle({ source_document_sha256: sha("other") }),
      );
    if (failure === "revision")
      await addin.mockApiJson("GET", bundleUrl, bundle({ revision: 2 }));
    if (failure === "span")
      await addin.mockApiJson("GET", bundleUrl, bundle({}, [[0, 100]]));
    if (failure === "overlap")
      await addin.mockApiJson(
        "GET",
        bundleUrl,
    ...
    if (failure === "API")
      await addin.mockApiError("GET", bundleUrl, 403, "secret internal error");
    if (failure === "snapshot")
      await page.evaluate(() => {
        (window as any).__SNAPSHOT_FAILURE__ = true;
      });
    if (failure === "binary hash")
      await page.evaluate(() => {
        (window as any).__SNAPSHOT_BYTES__ = "other";
      });
    await load(page);
    await blocked(page, addin);
...
test("Save-As during asynchronous load", async ({ addin, page }) => {
  ...
  const request = page.waitForRequest((request) =>
    request.url().includes("redline-bundle"),
  );
  await load(page);
  await request;
  await saveAs(page);
...
test("Save-As during the pre-apply digest window makes zero writes", async ({
  addin,
  page,
}) => {
  await setup(addin, page);
  ...
  await page.evaluate(() => {
    ...
    (window as any).__APPROVED_DIGEST_STARTED__ = () => started;
    (window as any).__RELEASE_APPROVED_DIGEST__ = release;
  ...
  await apply.evaluate((element) => (element as HTMLElement).click());
  await expect
    .poll(() =>
      page.evaluate(() => (window as any).__APPROVED_DIGEST_STARTED__()),
    )
    .toBe(true);
  await saveAs(page);
  await page.evaluate(() => (window as any).__RELEASE_APPROVED_DIGEST__());
  await blocked(page, addin);
});
...
for (const failure of [
  ...
  test(`before apply fails closed: ${failure}`, async ({ addin, page }) => {
    ...
    if (failure === "review revision")
      await addin.mockApiJson("GET", scopeUrl, { ...review, revision: 2 });
    if (failure === "review status")
      await addin.mockApiJson("GET", scopeUrl, {
        ...review,
    ...
    if (failure === "bundle replacement") {
      const value = bundle();
      value.actions[1]!.replacement_text = "changed without hash update";
      await addin.mockApiJson("GET", bundleUrl, value);
    }
    if (failure === "API") await addin.mockApiError("GET", scopeUrl, 404);
    if (failure === "identity during fetch")
      await page.route(bundleUrl, async (route) => {
        await saveAs(page);
        await route.fulfill({ json: bundle() });
      });
    ...
    await blocked(page, addin);
    await expect(page.getByRole("alert")).toContainText(
      "prior Word revisions remain reviewable",
    );
  });
}

test("selection change discards an asynchronous load", async ({
  addin,
  page,
}) => {
  await setup(addin, page);
  ...
  release();
  await expect(
    page.getByText("No succeeded executions available."),
  ).toBeVisible();

...
import {
  ...
  type PreparedApprovedRedlineAction,
} from "../../lib/approvedRedline";
import {
  readApprovedRedlineDocumentIdentity,
  readApprovedRedlineDocumentSnapshot,
  applyApprovedRedlineAction,
  type ApprovedRedlineDocumentIdentity,
} from "../../hooks/useWordDoc";

const canonical = canonicalApprovedRedlineJson;
const fail = (
  code: ConstructorParameters<typeof ApprovedRedlineValidationError>[0],
): never => {
...
function checkScope(
  ...
  if (
    ...
    scope.execution_id !== execution.id ||
    scope.matter_id !== execution.matter_id ||
    scope.document_id !== execution.document_id ||
    scope.document_version_id !== execution.document_version_id ||
    scope.document_content_sha256 !== execution.document_content_sha256
  )
    fail("scope-mismatch");
  ...
  if (error instanceof ApprovedRedlineValidationError) {
    switch (error.code) {
      case "document-identity":
        return "Cannot apply: Word document identity changed or is unavailable. Reopen the original saved document and reload the approved review.";
      case "superseded":
        return "Cannot apply: the review or bundle is no longer the approved revision. Obtain the current approved review and reload.";
      case "scope-mismatch":
        return "Cannot apply: project, matter, execution, or document scope does not match. Select the correct project and review.";
      case "source-mismatch":
        return "Cannot apply: the document version or file hash does not match. Open the reviewed source document and reload.";
      case "tampered":
        return "Cannot apply: the approved bundle failed its integrity check. Obtain a fresh approved bundle and reload.";
      default:
        return "Cannot apply: redline targets are missing, ambiguous, overlapping, or invalid. Request a new review of the current document.";
    }
  }
...
type Loaded = {
  scope: AiReviewScope;
  execution: AiExecutionSummary;
  identity: ApprovedRedlineDocumentIdentity;
  rawCanonical: string;
  actions: readonly PreparedApprovedRedlineAction[];
};
const selectClass =
  "liquid-glass-subtle rounded-lg p-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring";

export function ApprovedRedlinePanel({
  wordDocumentId,
}: {
  ...
  async function selectExecution(id: string): Promise<void> {
    ...
    try {
      ...
      const identity = readApprovedRedlineDocumentIdentity(wordDocumentId);
      const [scope, raw] = await Promise.all([
        getAiReviewScope(project, id),
        getApprovedRedlineBundle(project, id, 1),
      ]);
      if (ticket !== generation.current) return;
      checkScope(scope, execution, project);
      if (readApprovedRedlineDocumentIdentity(wordDocumentId) !== identity)
        fail("document-identity");
      const snapshot =
        await readApprovedRedlineDocumentSnapshot(wordDocumentId);
      if (ticket !== generation.current) return;
      const currentIdentity = snapshot.identity;
      const prepared = await prepareApprovedRedline({
        ...
        expected: {
          ...
          source_document_sha256: execution.document_content_sha256,
          document_identity: identity,
        },
        current: {
          document_identity: currentIdentity,
          text: snapshot.text,
      ...
      if (
        prepared.actions.some(
          (action) =>
            action.document_id !== execution.document_id ||
            action.document_version_id !== execution.document_version_id,
        )
      )
        fail("source-mismatch");
      const actions = [...prepared.actions].sort(
        (a, b) =>
          b.start - a.start ||
  ...
  async function apply(index: number): Promise<void> {
    if (!loaded || error || busy || applyLock.current || index !== completed)
      return;
    ...
    try {
      if (
        readApprovedRedlineDocumentIdentity(wordDocumentId) !== loaded.identity
      )
        fail("document-identity");
      const [scope, raw] = await Promise.all([
      ...
      if (
        canonical(scope) !== canonical(loaded.scope) ||
        canonical(raw) !== loaded.rawCanonical
      )
        fail("superseded");
      if (
        readApprovedRedlineDocumentIdentity(wordDocumentId) !== loaded.identity
      ...
      await applyApprovedRedlineAction(action, loaded.identity);
      if (ticket === generation.current) setCompleted(index + 1);
    } catch (reason) {
      if (ticket === generation.current)
        setError(
          `${safeMessage(reason)} Apply stopped; prior Word revisions remain reviewable. No retries are available in this session.`,
        );
    } finally {
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col gap-3 overflow-y-auto p-4">
      ...
      <p className="text-sm text-muted-foreground">
        Select a project and succeeded execution. Each approved action creates a
        reviewable Word revision.
      </p>
      <div className="flex flex-col gap-1 text-sm">
        <label htmlFor="approved-redline-project">Project</label>
        <select
          id="approved-redline-project"
          className={selectClass}
          value={project}
          disabled={applying || (!!loaded && !!error)}
      ...
      <div className="flex flex-col gap-1 text-sm">
        ...
        <select
          ...
          onChange={(event) => void selectExecution(event.target.value)}
        >
      ...
      {!busy && project && !executions.length && !error && (
        <p className="text-sm">No succeeded executions available.</p>
      )}
      {error && (
      ...
      {loaded?.actions.map((action, index) => (
        <article
          ...
          aria-label={`Redline ${action.action_id}`}
        >
          <GlassCardUI>
            <div className="flex flex-col gap-2 p-3">
              <p className="text-xs text-muted-foreground">
                Page {action.page} · {action.action_id}
              </p>
              ...
              <p role="status" className="text-sm">
                {index < completed
                  ...
                  : error
                    ? "Halted"
                    : applying && index === completed
                      ? "Applying…"
                      : index === completed
                        ? "Ready"
                        : "Pending — apply the preceding action first"}
              </p>
              {!error && index >= completed && (
                <PillButtonUI
                  tone="black"
                  type="button"
                  disabled={applying || index !== completed}
                  onClick={() => void apply(index)}
                >
                  Apply {action.action_id}
                </PillButtonUI>
              )}
            </div>
          </GlassCardUI>

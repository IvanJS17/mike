...
LiTT connects to the systems a legal team already works in — Slack and any
...
on the LiTT server at all: a user clicks **Add**, pastes the server URL (or
...
not implement dynamic client registration, so the person hosting LiTT creates
...
| Production | `https://<your-litt-host>/api` | `https://<your-litt-host>/api/user/…/oauth/callback` |
...
3. Add the callback,
   `https://<your-litt-host>/api/user/mcp-connectors/oauth/callback`, as a
...
and the toggle refuses to enable them: LiTT has no human-confirmation step
